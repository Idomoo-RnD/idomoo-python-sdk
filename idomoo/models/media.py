...
class Media(object):
    ...
    def __init__(self, key=None, val=None, alignment_scale_type='fit', alignment_scale=None, alignment_x_align=None, alignment_y_align=None, if_longer='cut', mute=False, fade_start=None, fade_end=None, volume=None, sidechain_compression_affect=False, duration_referrer=False, duration_addition=None, is_hidden=False):
    ...
    def val(self):
        ...
        The path to the content to be placed in the media placeholder. * For images and video use a path to external files or files held on Idomoo servers. More about paths can be found [here](https://academy.idomoo.com/support/solutions/articles/4000068850). * For color use this syntax: `\"rgb(###, ###, ###)\"`. Use 8bit color ranging from `0-255`.
    ...
    def val(self, val):
        ...
        if val is not None and not re.search('^(http|ual|pal|rgb)', val):
            raise ValueError("Invalid value for `val`, must be a follow pattern or equal to `/^(http|ual|pal|rgb)/`")
    ...
    def alignment_scale_type(self):
        ...
        How to scale the image or video media in case it's not the same dimensions as the placeholder. * Fit: scale the media up or down so it's completely visible in the video's resolution. If not the same aspect ratio, transparency is added around the image according to the alignment settings. * Fill: scale the media up or down so it completely fills the video. If not the same aspect ratio, the image is cropped according to the alignment settings.  * Custom: scale the media by a specific multiplier. Used with `\"alignment_scale\"`.  Colors fill the placeholder completely.  Media is always cropped inside the placeholder.
    ...
    def if_longer(self):
        ...
        Only used with video media assets.  If the video is shorter than the placeholder, what to do with the video. * Cut: cut the placeholder. Nothing is scene after the media is finished. * Loop: loop the video so it repeats. * Hold: hold the last frame of the video frozen.
    ...
    def mute(self):
        ...
        Only used with video media assets that contain audio.  If the video contains audio you don't want to be used, make this True.
    ...
    def fade_start(self):
        ...
        Only used with video media assets that contain audio.  Fade in of sound from no sound to the level specified in volume. How many seconds should the fade take. 0 means no fade.
    ...
    def fade_end(self):
        ...
        Only used with video media assets that contain audio.  Fade out the sound from the level specified in volume to no sound. How many seconds should the fade take. 0 means no fade.
    ...
    def sidechain_compression_affect(self):
        ...
        Only used with video media assets that contain audio.  Sidechain compression means other audio placeholders can change the level of the soundtrack's volume. Only placeholder that have `\"sidechain_compression_affect\":true` will affect the soundtrack's volume. True means this placeholder is one.
    ...
    def duration_referrer(self):
        ...
        Only used with video media assets.  This placeholder defines the duration of the scene. Only a single placeholder in a scene can use this. When the media of this placeholder is over + the `\"duration_addition\"` is over, the scene ends.
